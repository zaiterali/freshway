                  ...
                  <table id="catList" class="table">
                    <thead>
                      <tr>
                        ...
                        <th>Status</th>
                        <th>Cash</th>
                        <th>Print<br><small>Kitchen</small></th>
                    ...
                    <tbody class="table-border-bottom-0">
                      ...
                      <?php
                      $sql = "SELECT *, o.id as order_id FROM order_list o LEFT JOIN clients c ON o.client=c.id ORDER BY o.id DESC";
                      ...
                      if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) { ?>
                          <tr>
                            ...
                            <td class="fw-bold">
                              ...
                              if ($row['byclient'] == 1) { ?>
                                ...
                                <?php
                                if ($row['confirmed'] == 0) { ?>
                                  <button type="button" onclick="confirmOrder('<?php echo $row['order_id'] ?>', '<?php echo $row['phonenumber'] ?>')" class="btn btn-xs btn-outline-success waves-effect my-1">Confirm</button>
                                <?php }
                                ?>
                              <?php }
                              ?>
                            ...
                            <td>
                              <?php
                              if ($row['status'] == 1) { ?>
                                <button type="button" onclick="colectCash('<?php echo $row['order_id'] ?>')" class="btn btn-icon btn-info waves-effect waves-light">
                                  <span class="tf-icons mdi mdi-currency-usd"></span>
                                </button>
                              <?php
                              } else { ?>
                            ...
                            <td>
                              ...
                              if ($row['status'] == 1 or $row['status'] == 2) { ?>
                                <button type="button" onclick="printKitchen('<?php echo $row['order_id'] ?>')" class="btn btn-icon btn-info waves-effect waves-light">
                          ...
                          <?php
                            ...
                            <button type="button" onclick="cancelOrder('<?php echo $row['order_id'] ?>')" class="btn btn-icon btn-danger waves-effect waves-light">
                              <span class="tf-icons mdi mdi-delete-outline"></span>
                            </button></td>
                          <?php
                              }
...
<script>
  ...
  // Function 
  function confirmOrder(orderId, phone) {
    var order_id = orderId;
    var phone_number = phone;
    // Perform AJAX request to update SQL
    $.ajax({
      url: '../config/confirmorder.php', // Replace with your PHP file path
      method: 'POST',
      data: {
        order_id: order_id,
