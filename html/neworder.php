...
if (isset($_SESSION['roleId']) && isset($_SESSION['username'])) { ?>
  ...
  if (isset($_POST['startOrder'])) {
    ...
    $sql = "INSERT INTO order_list (user_id, client, order_note, kitchen_note, delivery_date) 
    VALUES ($userId, '$client', '$ordernote', '$kitchennote', '$deliverydate')";

    if (mysqli_query($conn, $sql)) {
      $lastInsertedId = mysqli_insert_id($conn);

      // Redirect to neworderitems.php with the order ID
      header("Location: ../html/neworderitems.php?client=$client&ordernote=$ordernote&orderid=$lastInsertedId&orderdate=$deliverydate");
    } else {
              ...
              <div class="card mb-2">
                <div class="card-body pt-2 mt-1">
                  <form id="clientCard" autocomplete="off" method="POST">
                    ...
                    <div class="row mt-2 gy-4">
                      <div class="col-md-6">
                        <div class="form-floating form-floating-outline">
                          <input class="form-control" list="clients" type="text" id="firstName" name="client" autofocus=" " required>
                          <datalist id="clients">
                            <?php
                            $sql = "SELECT id, companyname, contactperson FROM clients WHERE isactive=1";
                            $result = $conn->query($sql);
                            if ($result->num_rows > 0) {
                              while ($row = $result->fetch_assoc()) { ?>
                                <option value="<?php echo $row['id'] ?>-<?php echo $row['companyname'] ?>">
                                  <?php echo $row['contactperson'] ?></option>
                            <?php
                              }
                            }
                            ?>
                      ...
                      <div class="col-md-6">
                        <div class="form-floating form-floating-outline">
                          <input class="form-control" type="date" name="deliverydate" id="html5-date-input" value="<?php echo date('Y-m-d'); ?>">
                      ...
                      <div class="col-md-6">
                        <div class="form-floating form-floating-outline">
                          <textarea class="form-control h-px-100" id="exampleFormControlTextarea1" name="ordernote" placeholder="..."></textarea>
                          <label for="exampleFormControlTextarea1">Order Notes:</label>
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="form-floating form-floating-outline">
                          <textarea class="form-control h-px-100" id="kitNotes" name="kitchennote" placeholder="..."></textarea>
                    ...
                    <div class="mt-4">
                      <button type="submit" name="startOrder" class="btn btn-primary me-2 waves-effect waves-light">Start</button>
